...
4. Translate the extracted text accurately while maintaining the identified context.`;
  ...
  try {
    const response = await client.models.generateContent({
      model: 'gemini-3-flash-preview', // Basic text tasks, supports multimodal well
      contents: contents,
      config: {
    ...
    throw new Error('Failed to translate content. Please confirm file size is under 20MB and try again.');
  }
}
